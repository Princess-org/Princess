...
def test_size_of():
    assert parse("size_of int")              == Program(SizeOf(Identifier("int")))
    assert parse("size_of *int")             == Program(SizeOf(Ptr(right = Identifier("int"))))
    assert parse("size_of type struct { }")  == Program(SizeOf(Struct(body = StructBody(None))))
