...
bool test_compiler_print_ll;
 ...
 void _6dcc03b3_test_struct_lit() {
    ...
    res = _6dcc03b3_compile(str);
    str = ((Array){127, "\x0a""        type A = struct {\x0a""            a: int\x0a""            b: int\x0a""            c: int\x0a""        }\x0a""        let a = {a = 10} !A\x0a""    "});
    res = _6dcc03b3_compile(str);
    printf((((Array){3, "%s"}).value), (((Array){4, "OK\x0a"""}).value));
};
 ...
 void _6dcc03b3_test_builtins() {
    ...
    string res = _6dcc03b3_compile(str);
    str = ((Array){285, "\x0a""        let fp: File = open(\"bin/test\", \"wb+\")\x0a""        \x0a""        write(fp, \"some string\")\x0a""        let i = 20\x0a""        write(fp, *i)\x0a""\x0a""        rewind(fp)\x0a""\x0a""        var str: [12; char]\x0a""        read(fp, str)\x0a""        \x0a""        var i2: int\x0a""        read(fp, *i2)\x0a""        \x0a""        close(fp)\x0a""    "});
    res = _6dcc03b3_compile(str);
    str = ((Array){297, "\x0a""        let fp: File = open(\"bin/test_file_io_text\", \"w+\")\x0a""\x0a""        write_string(fp, \"This is a test\\n\", 10)\x0a""\x0a""        seek(fp, 0, SEEK_SET) // Same as rewind\x0a""\x0a""        var buffer: [20; char]\x0a""        read_line(fp, buffer)\x0a""        var num: int\x0a""        scanf(fp, \"%d\", *num)\x0a""\x0a""        close(fp)\x0a""\x0a""    "});
    res = _6dcc03b3_compile(str);
    printf((((Array){3, "%s"}).value), (((Array){4, "OK\x0a"""}).value));
};
