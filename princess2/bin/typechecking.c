...
DLL_EXPORT int typechecking_overload_score(typechecking_Type *a, vector_Vector *param_b) {
    ...
    for (int i = 0;(i < vector_length(param_b));(i += 1)) {
        ...
        typechecking_NamedParameter *left;
...
#include "builtins.c"
#include "debug.c"
DLL_EXPORT void typechecking_typecheck(parser_Node *node, scope_Scope *scope, string filename, string module);
#include "toolchain.c"
...
DLL_EXPORT void typechecking_p_main(Array args) {
    ...
    ((*_3700c937_int_literal).align) = ((*_3700c937_int_literal).size);
};
